...
export default function Productos() {
  ...
  const handleEliminar = async (id: number) => {
    if (!confirm('¿Eliminar este producto? Solo procederá si no tiene movimientos registrados.')) return
    try {
  ...
  const selectFiltroStyle: React.CSSProperties = {
    background: '#07101e', border: '1px solid rgba(56,139,221,0.2)',
    borderRadius: 6, padding: '4px 8px', fontSize: 11, color: '#c8ddef',
    ...
    <div style={{ minHeight: '100vh', display: 'flex', background: '#07101e', fontFamily: "'Inter', sans-serif", color: '#c8ddef' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <header style={{
          ...
          background: '#080e1c', position: 'sticky' as const, top: 0, zIndex: 30,
        }}>
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button
              ...
              style={{
                ...
                border: '1px solid rgba(245,158,11,0.28)', background: 'rgba(245,158,11,0.12)',
          ...
          <div style={{ background: '#0d1525', border: '1px solid rgba(56,139,221,0.1)', borderRadius: 10, overflow: 'hidden' }}>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", padding: '1px 8px', borderRadius: 20, background: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.22)', color: '#f59e0b' }}>
                  {productos.length}
                </span>
              </div>

              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <select value={empresaFiltro} onChange={e => setEmpresaFiltro(e.target.value)} style={selectFiltroStyle}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#07101e', border: '1px solid rgba(56,139,221,0.2)', borderRadius: 6, padding: '5px 10px', width: 200 }}>
                  <span style={{ color: '#4a7a9a', display: 'flex' }}><IconSearch /></span>
                  <input
                    type="text" placeholder="Buscar código o desc..."
                    value={busqueda} onChange={e => setBusqueda(e.target.value)}
                    style={{ background: 'transparent', border: 'none', outline: 'none', color: '#c8ddef', fontSize: 11, fontFamily: "'IBM Plex Mono', monospace", width: '100%' }}
                  />
                </div>
              </div>
            </div>
                          ...
                          <td style={{ ...td, textAlign: 'center' }}>
                            <div style={{ display: 'flex', justifyContent: 'center', gap: 6 }}>
                              <button
                                ...
                                style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 6, border: 'none', background: 'rgba(56,139,221,0.12)', color: '#60a5fa', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                              ...
                              <button
                                ...
                                style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 10px', borderRadius: 6, border: 'none', background: 'rgba(239,68,68,0.1)', color: '#f87171', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
