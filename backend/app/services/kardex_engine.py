...
def calcular_saldo_final(
    ...
    for codigo in df["Codigo"].unique():
        ...
        # ── Saldo inicial ──────────────────────────────────────────────
        # ✅ FIX: solo aplicar si fecha del saldo <= fecha del primer movimiento
        if saldo_ini:
            fecha_saldo      = saldo_ini.get("fecha")
            primer_mov_fecha = df.loc[mask, "Fecha"].min()

            aplica_saldo = True  # por defecto aplica

            if fecha_saldo and pd.notna(primer_mov_fecha):
                # Normalizar fecha saldo a date
                if isinstance(fecha_saldo, date_type):
                    fecha_saldo_cmp = fecha_saldo
                else:
                    fecha_saldo_cmp = pd.to_datetime(fecha_saldo).date()

                # Normalizar primer movimiento a date
                primer_fecha_cmp = (
                ...
                # El saldo solo aplica si su fecha es <= primer movimiento
                if fecha_saldo_cmp > primer_fecha_cmp:
                    aplica_saldo = False
                    print(
                        f"⚠️  Saldo inicial de {codigo} ignorado: "
                        f"fecha saldo ({fecha_saldo_cmp}) > "
                        f"primer movimiento ({primer_fecha_cmp})"
                    )
...
def calcular_metricas(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "saldo_final_costo": (
            float(d(ultima_fila["Saldo_Costo_Total"])) if ultima_fila is not None else 0.0
        ),
    }